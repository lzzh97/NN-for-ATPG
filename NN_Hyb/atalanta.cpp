...
int main(int argc, char *argv[]) //for windows
{
	...
	*/
	...
	strcpy(name, g_strCctPathFileName);
	if (strstr(name, "5315")!=nullptr){
		Apply_model = FALSE;
	}
...
int read_option(char option, char *array[], int i, int n)
{
	...
	{
	case 'w':
		printf("Lookup table disabled!\n");
		lookup = false;
	case 'k':
		printf("Learning Model applied!\n");
		Apply_model = true; break;
	case 'd':
		inputmode = 'd'; break;
#ifdef ISCAS85_NETLIST_MODE
	case 'I':
